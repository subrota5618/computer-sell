...
const AllBuyers = () => {
    ...
    const { data: buyers = [], isLoading, refetch } = useQuery({
        queryKey: ['buyers'],
        queryFn: () =>
            fetch(`https://resell-products-subrota22.vercel.app/buyers`, {
                headers: {
    ...
    const handleDelete = (buyerInfo) => {
        axios.delete(`https://resell-products-subrota22.vercel.app/buyers/${buyerInfo._id}`)
            .then(res => {
                if (res.status === 401 || res.status === 403) {
                    return navigate("/");
                }
    ...
    return (
        <>
            ...
            <h2 className='text-center my-2 text-2xl font-bold text-info'> 
            Total 
            <span className='mx-2'>  {
                buyers?.length 
            }</span>
