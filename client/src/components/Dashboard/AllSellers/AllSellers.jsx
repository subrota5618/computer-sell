...
const AllSellers = () => {
  ...
  const { data: sellers = [], isLoading, refetch } = useQuery({
    queryKey: ['sellers'],
    queryFn: () =>
      fetch(`https://resell-products.vercel.app/sellers`, {
        headers: {
  ...
  const handleDelete = (sellerInfo) => {
    axios.delete(`https://resell-products.vercel.app/sellers/${sellerInfo._id}`)
      .then(res => {
  ...
  const handleUpdate = (seller) => {
    axios.put(`https://resell-products.vercel.app/sellers/` , seller)
      .then(res => {
        if (res.status === 401 || res.status === 403) {
          return navigate("/");
        }
  ...
  return (
    <>
      ...
      <h2 className='text-center text-2xl my-2 font-bold text-info'> 
            Total 
            <span className='mx-2'> {
                sellers?.length 
            }</span>
