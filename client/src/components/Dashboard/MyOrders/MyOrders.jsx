...
const { data: orders = [email], isLoading  } = useQuery({
    queryKey: ['sellers'],
    queryFn: () =>
        fetch(`https://resell-products-subrota22.vercel.app/orders/${email}`, {
            headers: {
...
if (isLoading) {
    ...
    return (
        <>
            ...
            <h2 className='text-center text-2xl font-bold my-5 '>  Total {orders?.length ? orders?.length : "0"} orders sucess  </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 mx-5 gap-4">
                {
                    orders.map(order =>
                        <div className="card w-96 bg-base-100 shadow-xl" key={order._id}>
            ...
            {
                orders?.length === 0 &&
                <h2 className='text-2xl text-center my-36 font-bold'> Please add some orders to see your my orders page </h2>
            }
        </>
    );
};
