...
const MyProducts = () => {
    ...
    const { data: totalMyProducts = [email], isLoading, refetch } = useQuery({
        queryKey: ['sellers'],
        queryFn: () =>
            fetch(`https://resell-products-subrota22.vercel.app/products/${email}`, {
                headers: {
    ...
    const handleAddproduct = (id) => {
        axios.put(`https://resell-products-subrota22.vercel.app/products/${id}`)
            .then(res => {
    ...
    const hadleDelete = (deletdProduct) => {
        ...
        axios.delete(`https://resell-products-subrota22.vercel.app/products/${id}`)
            .then(res => {
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-10'>
                {
                    totalMyProducts.map(product =>
                        ...
                        <div className="card w-full bg-base-100 shadow-xl" key={product._id}>
                            <figure><img src={product.productImage} alt={product.productName} className='h-96' /></figure>
