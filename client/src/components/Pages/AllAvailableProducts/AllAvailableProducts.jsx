...
const AllAvailableProducts = () => {
    ...
    const { data: productItems = [], isLoading, refetch } = useQuery({
        queryKey: ['sellers'],
        queryFn: () =>
            fetch(`https://resell-products-subrota22.vercel.app/availAbleProducts/${categoryId}`, {
                headers: {
    ...
    React.useEffect(() => {
        //get all users 
        axios.get("https://resell-products-subrota22.vercel.app/users")
    ...
    const handleWishList = (productItem) => {
        ...
        axios.put(`https://resell-products-subrota22.vercel.app/wishList/`, wishtListInfo)
            .then(res => {
    ...
    const handleReport = (product) => {
        axios.put(`https://resell-products-subrota22.vercel.app/repot/${product._id}`)
            .then(res => {
                if (res.data.modifiedCount > 0) {
    ...
    return (
        <>
            ...
            <h2 className='text-center text-info font-bold text-2xl'>Total
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-7 my-10 mx-auto lg:grid-cols-3">
                {
                    productItems.map(product =>
                        <div className="card w-96 bg-base-100  hover:translate-y-8  transition duration-1000 shadow-xl" key={product._id}>
