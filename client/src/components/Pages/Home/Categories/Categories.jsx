...
const Categories = () => {
  ...
  const { data: categories, isLoading } = useQuery({
    queryKey: ['categories '],
    queryFn: () => fetch(`https://resell-products-subrota22.vercel.app/categories`, {
      method: "GET",
      headers: {
        ...
        isLoading ? <HashLoader style={{ margin: "30% 40%" }} color="#DE1068"></HashLoader> :
          <>
            <div>
              ...
              <p className='text-info font-bold p-10 text-xl'>
            ...
            <h2 className='text-center font-bold text-3xl text-info my-6'> There is a total of {categories.length} categories available for Computer . 
            ...
            <div className="grid gap-8 mx-2 my-5 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">

              {
                categories.map(category =>
                  <div className="card w-96 bg-base-100 shadow-xl" key={category._id}>
